...
from .common import PPTypes, PPConfig, PPInputData, PPInternalData, PPSimulation, PPPostSimulation
...
class PPInputData_3DDiscrete(PPInputData):
    ...
    def __load_from_file__(self):
        Logger.logToStdOut("info",'Loading input file... ' + self.ROOT + self.input_file, self.DOMAIN_MIN)
...
class PPInternalData_3DDiscrete(PPInternalData):
    ...
    def __init__(self, rng, ppKernels, ppConfig):
        ...
        Logger.logToStdOut("info",'Total GPU memory allocated:', PPTypes.INT_CPU(4 * (\
            ...
            while window.running if 'window' in locals() else True:
                ...
                if batch_mode is False:
                    ...
                    if self.do_screenshot:
                        window.save_image(ppConfig.ppData.ROOT + 'capture/screenshot_' + Logger.stamp() + '.png') ## Must appear before window.show() call
                        self.do_screenshot = False
                    window.show()
                if self.do_export:
