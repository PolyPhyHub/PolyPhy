...
class CliHelper:
    ...
    def parse_args():
        ...
        parser.add_argument("pipeline", type=str, choices=['2d_discrete','3d_discrete'], help="Run one of the pipelines")
        ...
        parser.add_argument('--distance-distribution', type=str, choices=['constant', 'exponential', 'maxwell-boltzmann'], help="Distance distribution (choose the type of distance distribution for agent interactions)")
        parser.add_argument('--directional-distribution', type=str, choices=['discrete', 'cone'], help="Directional distribution (choose the type of directional distribution for agent movement)")
        parser.add_argument('--directional-mutation', type=str, choices=['deterministic', 'stochastic'], help="Directional mutation (choose the type of directional mutation for agent behavior)")
        parser.add_argument('--deposit-fetching', type=str, choices=['nearest-neighbor', 'noise-perturbed-NN'], help="Deposit fetching (select the method for deposit fetching)")
        parser.add_argument('--agent-boundary-handling', type=str, choices=['wrap-around', 're-initialize-center', 're-initialize-randomly'], help="Agent boundary handling (specify how agents interact with boundaries)")
    ...
    def parse_values(ppConfig):
        ...
        if args.input_file:
            ppConfig.setter("input_file", args.input_file)
            if args.input_file:
        ...
        if args.distance_distribution:
            if args.distance_distribution == "constant":
                ppConfig.setter("distance_sampling_distribution",PPConfig.EnumDistanceSamplingDistribution.CONSTANT)
            elif args.distance_distribution == "exponential":
                ppConfig.setter("distance_sampling_distribution",PPConfig.EnumDistanceSamplingDistribution.EXPONENTIAL)
            elif args.distance_distribution == "maxwell-boltzmann":
                ppConfig.setter("distance_sampling_distribution",PPConfig.EnumDistanceSamplingDistribution.MAXWELL_BOLTZMANN)           
        if args.directional_distribution:
            if args.directional_distribution == "discrete":
                ppConfig.setter("directional_sampling_distribution",PPConfig.EnumDirectionalSamplingDistribution.DISCRETE)
            elif args.directional_distribution == "cone":
                ppConfig.setter("directional_sampling_distribution",PPConfig.EnumDirectionalSamplingDistribution.CONE)
        if args.directional_mutation:
            if args.directional_mutation == "deterministic":
                ppConfig.setter("directional_mutation_type",PPConfig.EnumDirectionalMutationType.DETERMINISTIC)
            elif args.directional_mutation == "stochastic":
                ppConfig.setter("directional_mutation_type",PPConfig.EnumDirectionalMutationType.PROBABILISTIC)
        if args.deposit_fetching:
            if args.deposit_fetching == "nearest-neighbor":
                ppConfig.setter("deposit_fetching_strategy",PPConfig.EnumDepositFetchingStrategy.NN)
            elif args.deposit_fetching == "noise-perturbed-NN":
                ppConfig.setter("deposit_fetching_strategy",PPConfig.EnumDepositFetchingStrategy.NN_PERTURBED)
        if args.agent_boundary_handling:
            if args.agent_boundary_handling == "wrap-around":
                ppConfig.setter("agent_boundary_handling",PPConfig.EnumAgentBoundaryHandling.WRAP)
            elif args.agent_boundary_handling == "re-initialize-center":
                ppConfig.setter("agent_boundary_handling",PPConfig.EnumAgentBoundaryHandling.REINIT_CENTER)
            elif args.agent_boundary_handling == "re-initialize-randomly":
                ppConfig.setter("agent_boundary_handling",PPConfig.EnumAgentBoundaryHandling.REINIT_RANDOMLY)
        if args.pipeline == "2d_discrete":
