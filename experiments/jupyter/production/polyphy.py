...
class PolyPhy:
    ...
    def parse_args(self):
        self.parser = argparse.ArgumentParser(description="PolyPhy")
        self.parser.add_argument('-f', '--input-file', type=str, help="Main input data file (string relative to the root directory)")
        self.parser.add_argument('-b', '--batch-mode', action='store_true', help="Enable batch mode (run simulations in batch mode)")
        self.parser.add_argument('-n', '--num-iterations', type=int, help="Number of iterations (specify the number of simulation iterations)")
        self.parser.add_argument('-d', '--sensing-dist', type=float, help="Sensing distance (set the agent's sensing distance in pixels)")
        self.parser.add_argument('-a', '--sensing-angle', type=float, help="Sensing angle (set the agent's sensing angle in degrees)")
        self.parser.add_argument('-e', '--sampling-expo', type=float, help="Sampling exponent (adjust the sampling exponent for data collection)")
        self.parser.add_argument('-s', '--step-size', type=float, help="Step size (define the step size for agent movement)")
        self.parser.add_argument('-D', '--data-deposit', type=str, help="Data deposit (specify the method for data deposition)")
        self.parser.add_argument('-A', '--agent-deposit', type=str, help="Agent deposit (specify the method for agent deposition)")
        self.parser.add_argument('-X', '--deposit-attenuation', type=float, help="Deposit attenuation (adjust the deposit attenuation factor)")
        self.parser.add_argument('-T', '--trace-attenuation', type=float, help="Trace attenuation (adjust the trace attenuation factor)")
        self.parser.add_argument('-Z', '--deposit-visualization', type=str, help="Deposit visualization (select the method for deposit visualization)")
        self.parser.add_argument('-Y', '--trace-visualization', type=str, help="Trace visualization (select the method for trace visualization)")
        self.parser.add_argument('--distance-distribution', type=str, choices=['constant', 'exponential', 'maxwell-boltzmann'], help="Distance distribution (choose the type of distance distribution for agent interactions)")
        self.parser.add_argument('--directional-distribution', type=str, choices=['discrete', 'cone'], help="Directional distribution (choose the type of directional distribution for agent movement)")
        self.parser.add_argument('--directional-mutation', type=str, choices=['deterministic', 'stochastic'], help="Directional mutation (choose the type of directional mutation for agent behavior)")
        self.parser.add_argument('--deposit-fetching', type=str, choices=['nearest-neighbor', 'noise-perturbed-NN'], help="Deposit fetching (select the method for deposit fetching)")
        self.parser.add_argument('--agent-boundary-handling', type=str, choices=['wrap-around', 're-initialize-center', 're-initialize-randomly'], help="Agent boundary handling (specify how agents interact with boundaries)")
